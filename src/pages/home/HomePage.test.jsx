...
describe("HomePage component", () => {
  ...
  it("Display the products correct", async () => {
    ...
    const productConatiner = await screen.findAllByTestId("product-container");
    expect(productConatiner.length).toBe(2);

    expect(
      within(productConatiner[0]).getByText(
        "Black and Gray Athletic Cotton Socks - 6 Pairs",
      ),
    ).toBeInTheDocument();

    expect(
      within(productConatiner[1]).getByText("Intermediate Size Basketball"),
    ).toBeInTheDocument();
  });
